'use client'

import type { FC } from 'react'
import { useNotes } from '@/hooks/crdt/use-notes'
import { NotesToolbarEditor } from '@/components/NotesToolbar'

import '@/app/prose.css'

export const Notes: FC<{ onReady?: () => void }> = ({ onReady }) => {
  const { viewRef, editor, isEditable, setEditor } = useNotes({ onReady })

  return (
    <div className='absolute inset-0 flex flex-col bg-white text-black'>
      {isEditable && <NotesToolbarEditor getView={() => viewRef.current} editorEl={editor} />}
      <div ref={setEditor} className='h-full w-full flex-1 overflow-auto p-5' />
    </div>
  )
}

export default Notes
