  ...
  return (
    <div className='pointer-events-none absolute inset-0 z-50 overflow-hidden'>
      {reactions.map((r) => (
        <div
          key={r.id}
          className='animate-float-up absolute bottom-20 flex flex-col items-center opacity-0'
          style={{
            left: `${r.x}%`,
        ...
        >
          ...
          <div className='mt-1 rounded-full bg-black/40 px-1.5 py-0.5 text-[10px] font-bold whitespace-nowrap text-white backdrop-blur-sm'>
            {r.senderName}
          </div>
        </div>
      ))}
