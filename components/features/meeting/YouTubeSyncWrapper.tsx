...
export function YouTubeSyncWrapper({ roomName, isAdmin, onClose, initialUrl }: YouTubeSyncWrapperProps) {
    ...
    useEffect(() => {
        ...
        const handleData = (payload: Uint8Array, participant?: RemoteParticipant, kind?: DataPacket_Kind) => {
            try {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-black relative animate-in fade-in zoom-in duration-300">
            ...
            {/* Error State */}
            {error && (
                <div className="absolute inset-0 flex flex-col items-center justify-center z-30 bg-black/80 p-6 text-center animate-in fade-in">
                    ...
                    {isAdmin && (
                        <button
                            onClick={onClose}
                            className="mt-6 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors text-sm font-medium"
                        >
                            Close Player
                        </button>
                    )}
            ...
            {isAdmin && isReady && !error && (
                <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center pointer-events-none">
                    <div className="pointer-events-auto bg-black/60 backdrop-blur top-full rounded-full px-4 py-2 flex items-center gap-4 border border-white/10 shadow-xl transform transition-transform hover:scale-105">
                        <button
            ...
            {isAdmin && !playing && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <div className="bg-black/40 backdrop-blur-sm p-4 rounded-xl border border-white/10 flex flex-col items-center gap-2 animate-pulse">
                        <Play className="fill-white text-white w-8 h-8" />
                        <p className="text-white font-medium text-sm">You are controlling playback</p>
                    </div>
                </div>
            )}
            ...
            <div className="flex-1 w-full h-full relative pointer-events-auto">
                {url && ReactPlayer.canPlay(url) ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-black">
                        <p className="text-white/50 text-sm">Waiting for valid video URL...</p>
                    </div>
                )}
