  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg =
          (data && (data.error || data.message)) || `Login failed with status ${res.status}`
        throw new Error(msg)
      }
      ...
      <FieldGroup>
        ...
        <Field>
          ...
          <Input
            ...
            className='border-slate-700 bg-slate-950 text-slate-50 placeholder:text-slate-500 focus-visible:ring-slate-500'
            required
            value={username}
        ...
        <Field>
          <Button
            type='submit'
            disabled={loading}
            className='w-full bg-emerald-500 font-semibold text-slate-950 hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-70'
          >
