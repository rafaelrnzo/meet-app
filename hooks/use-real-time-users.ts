...
import type { Users } from '@/feat/users/dto'

export type RoomEventType = 'connected' | 'user_updated' | 'ping'
...
const ALL_NAMED_EVENT_TYPES: RoomEventType[] = ['connected', 'user_updated', 'ping']

export function applyRoomEventToActiveRooms(
  activeRooms: ActiveRoom[],
  event: RoomSSEEvent
): ActiveRoom[] {
  const roomName = event.data?.room_id || 'unknown-room'

  const participantCount = event.data?.participant_count ?? event.data?.participants ?? 0

  const isRoomStateEvent = event.type === 'user_updated'
  ...
  if (participantCount <= 0 && roomName !== 'unknown-room') {
  ...
  return [
    ...activeRooms,
    {
      sid: roomName,
      name: roomName,
      num_publishers: event.data?.num_publishers ?? 0,
...
export function useRealTimeUsers(
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(`${baseUrl}/admin/users/events?${params.toString()}`)
    ...
    ALL_NAMED_EVENT_TYPES.forEach((eventType) => {
      eventSource.addEventListener(eventType, (event) => {
        try {
          ...
          const data: RoomSSEEvent = raw?.type ? raw : { type: eventType, data: raw }
          handleParsedEvent(data, setStatus, setLastEvent, onUpdateRef)
        } catch (err) {
